...
/* 달력 아이콘 */
[data-testid="stDateInput"] svg { fill: rgba(255,255,255,0.5) !important; }
...
/* ── 홈으로 버튼 ── */
#surit-back-btn + div .stButton button {
    background: rgba(255,255,255,0.08) !important;
...
#surit-back-btn + div .stButton button:hover {
...
if st.session_state.menu != "home":
    st.markdown('<div id="surit-back-btn"></div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════
    ...
    # ── 생년월일 입력 (업로드 전에 표시) ──
    st.markdown("""
    <div style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:12px;
                padding:14px 16px;margin-bottom:6px;">
        <div style="font-size:0.88rem;font-weight:700;color:rgba(255,255,255,0.9);margin-bottom:3px;">
            고객 생년월일 <span style="font-size:0.78rem;font-weight:400;color:rgba(255,255,255,0.4);">(선택)</span>
        </div>
...
설명, 주석, 마크다운 백틱, 전후 텍스트 일체 금지."""
    ...
    if prescription_end_details:
        ...
        if active_prescs:
            ...
            st.markdown(f"""
            <div style="background:rgba(239,68,68,0.1);border:1.5px solid rgba(248,113,113,0.35);
                        border-radius:12px;padding:14px 16px;margin-bottom:10px;">
                <div style="font-size:0.7rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;
                            color:#f87171;margin-bottom:6px;">💊 복약 중 — 처방 종료 후 가입 가능</div>
                <div style="font-size:0.88rem;font-weight:800;color:#fca5a5;margin-bottom:10px;">
                    최소 가입 가능 날짜: <span style="color:#a5b4fc;">{latest_avail["available"]}</span>
                </div>
                <table style="width:100%;border-collapse:collapse;background:rgba(255,255,255,0.05);border-radius:8px;overflow:hidden;">
                    <tr style="background:rgba(255,255,255,0.08);font-size:0.72rem;font-weight:700;color:rgba(255,255,255,0.55);">
                ...
                <div style="margin-top:8px;font-size:0.74rem;color:rgba(252,165,165,0.7);">
            ...
            st.markdown(f"""
            <div style="background:rgba(16,185,129,0.1);border:1px solid rgba(74,222,128,0.3);
                        border-radius:12px;padding:10px 16px;margin-bottom:10px;">
                <div style="font-size:0.8rem;font-weight:700;color:#4ade80;">
                    ✅ 3개월 이내 처방 {len(completed_prescs)}건 — 모두 복약 완료 (투약 관련 Q1 면제 가능)
                </div>
                <div style="font-size:0.75rem;color:rgba(134,239,172,0.7);margin-top:3px;">
        ...
        if analysis_product_type == "간편심사 (유병자 3-5-5 기준)" and drug_change_summary:
            if ai_res.get("drug_change_hit"):
                ...
                <div style="background:rgba(239,68,68,0.1);border:1.5px solid rgba(248,113,113,0.35);
                    ...
                    <div style="font-size:0.7rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#f87171;margin-bottom:4px;">
                        💊 처방약 변경 감지 — 간편심사 Q1 해당
                    </div>
                    <div style="font-size:0.84rem;font-weight:700;color:#fca5a5;margin-bottom:4px;">
                        3개월 이내 처방약 변경/추가 → 가입 불가
                    </div>
                    <div style="font-size:0.79rem;color:rgba(252,165,165,0.85);line-height:1.7;">{drug_reason}</div>
                    <div style="margin-top:7px;font-size:0.73rem;color:#fca5a5;background:rgba(239,68,68,0.12);border-radius:8px;padding:6px 10px;">
