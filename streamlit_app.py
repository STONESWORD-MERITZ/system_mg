...
# ══════════════════════════════════════════
    ...
    if run_btn:
      with st.spinner("📊 PDF 파싱 및 AI 분석 중..."):
        ...
        # ── 질병코드별 통원횟수 집계 (건강체 Q4 "계속하여 7일 이상 치료" 판단용) ──
        # 7일이상치료 기준: 투약일수가 아닌, 동일 질병코드 통원횟수 7회 이상
        ...
        for _code, _s in disease_stats.items():
            ...
            if _visits_in_10y:
                ...
                _first = min(_visits_in_10y)
                _last  = max(_visits_in_10y)
                _name  = _s.get("name", "")[:15]
                visit_count_lines.append(
                    f"[통원집계] {_code} {_name} 10년내통원{_cnt}회 ({_first}~{_last})"
                )
        if visit_count_lines:
            raw_text = "[10년내 질병코드별 통원횟수 집계 — Q4 7일이상치료 판단 기준]\n" \
...
Q4. 최근 10년({d_10y} 이후) — 태그 [IN_10Y] 항목만:
    - 입원
    - 수술 (제왕절개 포함)
    - 계속하여 7일 이상 치료 ★ = 동일 질병코드(KCD) 기준 통원횟수 7회 이상 ([통원집계]에서 10년내통원7회 이상인 코드만 해당, 투약일수 기준 아님)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
[5단계: Q3(건강체) 추가검사 판단 + 간편심사 Q3 면제 + Q4 면제 — 과잉 고지 방지]
━━━━━━━━━━━━━━━━━━━━━━━━━━
